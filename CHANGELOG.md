...
**Every release from 0.0.1 through 0.14.0 carries this defect.** This is a backport onto the
0.12 line so that a deployment bounded below 0.13 can take the fix without also taking mTLS
(0.13.0) or token exchange (0.14.0). The same fix ships on the 0.14 line as 0.14.1.

**This is a behaviour change, not only a fix.** A client that sends no `code_challenge` is now
...
start; on this package's client half it is already in use.

The consent path routes through the same request construction and is covered — consent is not a
way around it.
