...
struct OAuthConsentTests {
    ...
    func testConsentApproveReturnsCode() async throws {
        let storage = try OAuthStorage(path: ":memory:")
        let server = OAuthServer(storage: storage, issuer: "http://localhost:8080", scopesSupported: ["mcp:tools", "mcp:resources", "mcp:prompts"], served: .core, resourceIdentity: .colocated,
